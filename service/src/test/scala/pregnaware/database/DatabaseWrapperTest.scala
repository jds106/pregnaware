...
import pregnaware.user.entities.WrappedFriendToBe
...
class DatabaseWrapperTest extends UnitSpec {
  ...
  "User" should "add friend" in {
    ...
    val friendToBe : WrappedFriendToBe = Await.result(userWrapper.addFriend(user1.userId, user2.userId), timeout)
    ...
    val confirmedFriend = Await.result(userWrapper.confirmFriend(user2.userId, user1.userId), timeout)
    confirmedFriend.userId should be (user1.userId)
    confirmedFriend.displayName should be ("TEST_1")

    val user1PostFriendAccept = Await.result(userWrapper.getUser("TEST_EMAIL_1"), timeout).get
    val user2PostFriendAccept = Await.result(userWrapper.getUser("TEST_EMAIL_2"), timeout).get
